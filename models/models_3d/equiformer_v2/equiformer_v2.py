...
class EquiformerV2(torch.nn.Module):
    """
    ...
    def forward(self, z, pos, batch):
        ...
        for i in range(self.num_resolutions):
            if self.num_resolutions == 1:
                x.embedding[:, offset_res, :] = self.sphere_embedding(z)
            else:
        ...
        if self.share_atom_edge_embedding and self.use_atom_edge_embedding:
            source_element = z[edge_index[0]]  # Source atom atomic number
            target_element = z[edge_index[1]]  # Target atom atomic number
        ...
        energy = node_energy.embedding.narrow(1, 0, 1).squeeze(1)
        energy = scatter(energy, batch, dim=0)
        # node_energy = node_energy.embedding.narrow(1, 0, 1)
        # energy = torch.zeros(batch.shape[0], device=node_energy.device, dtype=node_energy.dtype)
