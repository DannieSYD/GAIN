...
class TransformerConv(MessagePassing):
    ...
    def message(
        self, query_i, key_j, value_j, edge_attr, edgeweight, emb, index, ptr, size_i
    ):
...
class ClofNet(torch.nn.Module):
    ...
    def forward(self, z, pos, batch):
        ...
        edge_cross = edge_cross / (
            (torch.sqrt(torch.sum((edge_cross) ** 2, 1).unsqueeze(1))) + self.eps
        )
        ...
        for i in range(self.num_layers):
            z_emb = self.message_layers[i](
                z_emb, edge_index, edge_attr=edgefeature, edgeweight=edgeweight
            )
            edge_embed = self.message_layers[i].edge_embed

        # s = self.last_layer(z_emb)
        s = scatter(z_emb, batch, dim=0)

        vec_coff = self.vec_lin(edge_embed)
        vec_output = (
            edge_diff * vec_coff[:, :1]
            + edge_cross * vec_coff[:, 1:2]
            + edge_vertical * vec_coff[:, 2:3]
        )
        vec_output = scatter(vec_output, edge_index[1], dim=0, reduce="sum")  # [N, 3]
        vec_output = scatter(vec_output, batch, dim=0)  # [G, 3]
