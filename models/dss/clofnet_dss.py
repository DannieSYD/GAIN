...
class ClofNet(torch.nn.Module):
    ...
    def build_avg_conf(self, data, conf_node_idx, reduce="mean"):
        ...
        batch_avg = torch.tensor([value_map[x.item()] for x in temp])

        # indices = torch.arange(len(conf_node_idx))
        # first_indices = scatter(indices, conf_node_idx.cpu(), dim=0, reduce='min')
        # z_avg = z[first_indices]
        # pos_avg = pos[first_indices]

        z_avg = scatter(z, conf_node_idx, dim=0, reduce="mean")
        pos_avg = scatter(pos, conf_node_idx, dim=0, reduce=reduce)

        return z_avg, pos_avg, batch_avg
    ...
    def block_call(self, i, x, data_dict, batch):
        edge_index = data_dict['edge_index']
        edgefeature = data_dict['edgefeature']
        edgeweight = data_dict['edgeweight']
        z_emb = self.message_layers[i](x, edge_index, edge_attr=edgefeature, edgeweight=edgeweight)
        return z_emb
