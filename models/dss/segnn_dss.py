...
from ..models_3d.SEGNN.balanced_irreps import BalancedIrreps, WeightBalancedIrreps
from ..models_3d.SEGNN.instance_norm import InstanceNorm
from ..models_3d.SEGNN.node_attribute_network import NodeAttributeNetwork
from ..models_3d.SEGNN.o3_building_blocks import O3TensorProduct, O3TensorProductSwishGate


class SEGNNModel(nn.Module):
    def __init__(self, input_features, output_features, hidden_features, N, norm, lmax_h, lmax_pos=None, pool="avg", edge_inference=False, cutoff=5.0):
    ...
    def preprocess(self, data):
        ...
        s = x[:, :self.input_features // 2]
        v = x[:, self.input_features // 2:].reshape(len(s), 3, self.input_features // 2)
    ...
    def block_call(self, i, s, v, pos, data_dict, batch):
        x = torch.cat((s, v.reshape(len(s), 3*self.input_features // 2)), dim=1)
        layer = self.blocks[i]
        edge_index = data_dict['edge_index']
        edge_dist = data_dict['edge_dist']
    ...
    def postprocess(self, s, v, data_dict, batch):
        x = torch.cat((s, v.reshape(len(s), 3 * self.input_features // 2)), dim=1)
        node_attr = data_dict['node_attr']
        # Output head
