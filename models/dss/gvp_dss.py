...
class GVP_GNN(nn.Module):
    ...
    '''
    def __init__(self, edge_cutoff=4.5, num_rbf=16, hidden_dim=128, num_layers=5, max_atomic_num=100):
        
        super().__init__()
        activations = (F.relu, None)
        self.edge_cutoff = edge_cutoff
        self.num_rbf = num_rbf
        self.dim = (hidden_dim, hidden_dim)
        self.num_layers = num_layers
        
        self.embed = nn.Embedding(max_atomic_num, hidden_dim, padding_idx=0)
        ...
        self.W_v = nn.Sequential(
            LayerNorm((hidden_dim, 0)),
            GVP((hidden_dim, 0), self.dim,
                activations=(None, None), vector_gate=True)
        )
        
        self.blocks = nn.ModuleList(
                GVPConvLayer(self.dim, _DEFAULT_E_DIM,
    ...
    def preprocess(self, data):
        ...
        edge_s, edge_v = _edge_features(pos, edge_index,
                                        D_max=self.edge_cutoff, num_rbf=self.num_rbf, device=x.device)
        ...
        return {
            ...
            'edge_v': edge_v,
            'h_E': h_E,
        }, h_V[0], h_V[1].permute(0, 2, 1)

    def block_call(self, i, s, v, data_dict, batch):
        h_V = (s, v.permute(0, 2, 1))
        h_E = data_dict['h_E']
        edge_index = data_dict['edge_index']
        h_V = self.blocks[i](h_V, edge_index, h_E)
        return h_V[0], h_V[1].permute(0, 2, 1)

    def postprocess(self, s, v, batch):
        h_V = (s, v.permute(0, 2, 1))
        out = self.W_out(h_V)
        return torch_scatter.scatter_mean(out, batch, dim=0)


    # def forward(self, data, scatter_mean=True, dense=True):
    #     '''
    ...
    #
    #     out = self.W_out(h_V)
    #     torch_scatter.scatter_mean(out, batch_id, dim=0)
    #
    #     return out
