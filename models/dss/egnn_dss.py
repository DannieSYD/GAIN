...
class EGNN_DSS(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.token_emb = nn.Embedding(num_tokens, hidden_dim) if exists(num_tokens) else None
        self.pos_emb = nn.Embedding(num_positions, hidden_dim) if exists(num_positions) else None
        ...
        for ind in range(num_layers):
            ...
            self.layers.append(nn.ModuleList([
                GlobalLinearAttention(dim = hidden_dim, heads = global_linear_attn_heads, dim_head = global_linear_attn_dim_head) if is_global_layer else None,
                EGNN(dim = hidden_dim, edge_dim = (edge_dim + adj_dim), norm_feats = True, **kwargs),
            ]))
    ...
    def preprocess(self, data, conf_node_idx=None, avg=False, reduce="mean"):
        if not avg:
            z, pos, batch = data.x[:, 0], data.pos, data.batch
        else:
            z, pos, batch = self.build_avg_conf(data, conf_node_idx, reduce=reduce)
    ...
    def postprocess(self, z_emb, batch):
        return scatter(z_emb, batch, dim=0)

    def build_blocks(self):
        for i in range(self.num_layers):
            block = self.EGNN(dim=self.hidden_dim)
            self.blocks.append(block)

    def block_call(self, i, x, vec, data_dict, batch):
        edge_feature = data_dict['edgefeature']
        feat, coor = self.blocks[i](i, x, vec, batch)
        return z_emb
