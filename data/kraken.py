...
class CommonKraken:
    ...
    def descriptors(self):
        return [
            'E_oxidation', 'E_reduction', 'E_solv_cds', 'E_solv_elstat', 'E_solv_total', 'Pint_P_int', 'Pint_P_max',
            'Pint_P_min', 'Pint_dP', 'dipolemoment', 'efg_amp_P', 'efgtens_xx_P', 'efgtens_yy_P', 'efgtens_zz_P',
            'fmo_e_homo', 'fmo_e_lumo', 'fmo_eta', 'fmo_mu', 'fmo_omega', 'fukui_m', 'fukui_p', 'nbo_P', 'nbo_P_ra',
            'nbo_P_rc', 'nbo_bd_e_avg', 'nbo_bd_e_max', 'nbo_bd_occ_avg', 'nbo_bd_occ_min', 'nbo_bds_e_avg',
            'nbo_bds_e_min', 'nbo_bds_occ_avg', 'nbo_bds_occ_max', 'nbo_lp_P_e', 'nbo_lp_P_occ', 'nbo_lp_P_percent_s',
            'nmr_P', 'nmrtens_sxx_P', 'nmrtens_syy_P', 'nmrtens_szz_P', 'nuesp_P', 'qpole_amp', 'qpoletens_xx',
            'qpoletens_yy', 'qpoletens_zz', 'somo_ra', 'somo_rc', 'sphericity', 'spindens_P_ra', 'spindens_P_rc',
            'sterimol_B1', 'sterimol_B5', 'sterimol_L', 'sterimol_burB1', 'sterimol_burB5', 'sterimol_burL',
            'surface_area', 'vbur_far_vbur', 'vbur_far_vtot', 'vbur_max_delta_qvbur', 'vbur_max_delta_qvtot',
            'vbur_near_vbur', 'vbur_near_vtot', 'vbur_ovbur_max', 'vbur_ovbur_min', 'vbur_ovtot_max', 'vbur_ovtot_min',
            'vbur_qvbur_max', 'vbur_qvbur_min', 'vbur_qvtot_max', 'vbur_qvtot_min', 'vbur_ratio_vbur_vtot', 'vbur_vbur',
            'vbur_vtot', 'vmin_r', 'vmin_vmin', 'volume']
    ...
    def _process_molecules(self, return_molecule_lists=False):
        ...
        for ligand_id in tqdm(ligand_ids):
            ...
            for conformer_id in conformer_ids:
                ...
                data.mol = mol
                data.y = torch.Tensor(
                    [conformer_properties[descriptor] for descriptor in self.descriptors]).unsqueeze(0)
            ...
            y.append(torch.Tensor([boltz_avg_properties[descriptor] for descriptor in self.descriptors]))
            if return_molecule_lists:
...
class KrakenV2(CommonKraken, EnsembleMultiPartDatasetV2):
    ...
    def processed_file_names(self):
        return 'KrakenV2_processed.pt' if self.max_num_conformers is None \
